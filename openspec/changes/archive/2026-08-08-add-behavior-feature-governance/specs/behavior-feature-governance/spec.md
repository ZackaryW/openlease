...
The committed behavior mapping SHALL declare a closed set of stable target identifiers, with each target resolving to one independently executable capability root. Runtime impact selection SHALL choose only declared targets and SHALL NOT invent executable command text or undeclared target names.
...
The repository SHALL expose an affected behavior command and a separately configured complete audit command. The complete audit SHALL execute every declared capability root regardless of affected-path selection, and repository acceptance SHALL require that audit to pass.
